...
combinations in the `Linux distribution packages` GitHub Actions workflow.
...
```sh
# Fedora
sudo dnf install ./git-hooks-ext-0.2.0-1.fc44.*.rpm
# Arch Linux
sudo pacman -U ./git-hooks-ext-0.2.0-1-x86_64.pkg.tar.zst
# Alpine (verify the release checksum first; no trusted APK repository yet)
sudo apk add --allow-untrusted ./git-hooks-ext-0.2.0-r0-alpine3.24-*.apk
```
...
reports it. This release is `0.2.0`, licensed under GPL-2.0-only.
...
archive is `dist/git-hooks-ext-0.2.0.tar.gz`. Its formula contains an absolute
...
```sh
make package-deb
sudo apt install ./dist/debian/git-hooks-ext_0.2.0-1_*.deb
```
